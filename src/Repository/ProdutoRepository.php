...
class ProdutoRepository
{
    ...
    {
        $sql1 = "SELECT * FROM produtos WHERE tipo = 'Doce' ORDER BY preco";
        $statement = $this->pdo->query($sql1);
        $produtosDoce = $statement->fetchAll(PDO::FETCH_ASSOC);

        $dadosDoce = array_map(function ($doce) {
    ...
    {
        $sql2 = "SELECT * FROM produtos WHERE tipo = 'Salgado' ORDER BY preco";
        $statement = $this->pdo->query($sql2);
        $produtosSalgado = $statement->fetchAll(PDO::FETCH_ASSOC);

        $dadosSalgado = array_map(function ($salgado) {
